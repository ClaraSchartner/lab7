...
ridge$grid <- function(x,y, len=NULL, search="grid"){
   data.frame(lambda=c(1,3,10,11,15,30))
    
}
#ridge$grid <- ridge.grid

#the fit element
Fit <- function(y, x, lambda, wts, param, lev, last, classProbs, ...){
  library(lab7)
    ridgereg(y=y, x=x, lambda=param$lambda) 
}
ridge$fit <- Fit
...
ridge.train <- train(y=training$crim, x=training[,-1], method=ridge, trControl = fitControl)
